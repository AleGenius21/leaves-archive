...
(function (global) {
    const LeavesUtils = {
         ...
         */
        formatDateDDMMYY: function (dateString) {
         ...
         */
        formatDateISO: function (dateInput) {
            if (!dateInput) return '';
         ...
         */
        formatDateDDMMYYYY: function (dateInput) {
            if (!dateInput) return '';
            const date = new Date(dateInput);
            if (isNaN(date.getTime())) return '';
            const day = String(date.getDate()).padStart(2, '0');
         ...
         */
        formatDateItalian: function (dateString) {
            if (!dateString) return '';
            const date = new Date(dateString + 'T00:00:00');
            if (isNaN(date.getTime())) return dateString;
         ...
         */
        formatDayDDMMYY: function (dateString) {
            if (!dateString) return '';
            const date = new Date(dateString + 'T00:00:00');
            if (isNaN(date.getTime())) return dateString;
         ...
         */
        formatTimeToHHMM: function (timeString) {
         ...
         */
        normalizeQuantity: function (value, unit) {
         ...
         */
        parseColorToRgb: function (color, rootSource) {
         ...
         */
        getRelativeLuminance: function (color, rootSource) {
            const rgb = this.parseColorToRgb(color, rootSource);
            if (!rgb) return 0.5;
         ...
         */
        getTextColorForBackground: function (bgColor, rootSource) {
            const luminance = this.getRelativeLuminance(bgColor, rootSource);
            return luminance > 0.45 ? '#000000' : '#ffffff';
        }
    };

    // Esponi globalmente
    global.LeavesUtils = LeavesUtils;
